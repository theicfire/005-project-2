...
public class Client {
	...
	public static void handleConnectionMessage(ConnectionMessage message) {
		if (message.type == ConnectionMessage.types.CONNECT){
			loggedIn(message.getFromUsername());
		} else if (message.type == ConnectionMessage.types.DISCONNECT){
			loggedOut(message.getFromUsername());
		}
	}
	...
	/* Called by handleConnectionMessage, when a user logs in, and a CONNECT message
	 * is sent to the client from the server. Logs the user in on the GUI. 
	 */
	private static void loggedOut(String fromUsername) {
		// TODO Auto-generated method stub
	}
	
	/* Called by handleConnectionMessage, when a user logs out, and a DISCONNECT
	 * is sent to the client from the server. Logs the user out on the GUI. 
	 */
	private static void loggedIn(String fromUsername) {
		// TODO Auto-generated method stub
